...
def analyze_cluster_composition(metadata, output_dir, method):
    ...
    valid_clusters = [c for c in metadata['cluster'].unique() if c != -1]
    ...
    # === NEW: Generate presence/absence matrices ===
    # Format: ROWS = clusters, COLUMNS = genomes/hosts
    
    # 1. Cluster presence/absence matrix for genomes
    unique_genomes = sorted(metadata['genome_id'].unique())
    valid_clusters_sorted = sorted(valid_clusters)
    
    # Create matrix: rows = clusters, columns = genomes
    genome_cluster_matrix = pd.DataFrame(0, index=valid_clusters_sorted, columns=unique_genomes)
    
    for genome_id in unique_genomes:
        genome_clusters = metadata[metadata['genome_id'] == genome_id]['cluster'].unique()
        # Only include valid clusters (exclude -1 noise)
        genome_clusters = [c for c in genome_clusters if c != -1]
        for cluster_id in genome_clusters:
            genome_cluster_matrix.loc[cluster_id, genome_id] = 1
    
    # Save genome presence/absence matrix
    genome_matrix_file = os.path.join(output_dir, f'{method}_cluster_genome_presence_matrix.tsv')
    genome_cluster_matrix.to_csv(genome_matrix_file, sep='\t')
    logging.info(f"Saved cluster-genome presence/absence matrix (rows=clusters, cols=genomes): {genome_matrix_file}")
    
    # 2. Cluster presence/absence matrix for hosts
    unique_hosts = sorted(metadata['host'].unique())
    
    # Create matrix: rows = clusters, columns = hosts
    host_cluster_matrix = pd.DataFrame(0, index=valid_clusters_sorted, columns=unique_hosts)
    
    for host in unique_hosts:
        host_clusters = metadata[metadata['host'] == host]['cluster'].unique()
        # Only include valid clusters (exclude -1 noise)
        host_clusters = [c for c in host_clusters if c != -1]
        for cluster_id in host_clusters:
            host_cluster_matrix.loc[cluster_id, host] = 1
    
    # Save host presence/absence matrix
    host_matrix_file = os.path.join(output_dir, f'{method}_cluster_host_presence_matrix.tsv')
    host_cluster_matrix.to_csv(host_matrix_file, sep='\t')
    logging.info(f"Saved cluster-host presence/absence matrix (rows=clusters, cols=hosts): {host_matrix_file}")
    
    # === Additional summary statistics for matrices ===
    
    # Genome matrix stats (note: axis changed since we transposed the matrix)
    genome_stats = {
        'total_genomes': len(unique_genomes),
        'total_clusters': len(valid_clusters_sorted),
        'avg_genomes_per_cluster': genome_cluster_matrix.sum(axis=1).mean(),  # sum across genomes for each cluster
        'max_genomes_per_cluster': genome_cluster_matrix.sum(axis=1).max(),
        'min_genomes_per_cluster': genome_cluster_matrix.sum(axis=1).min(),
        'avg_clusters_per_genome': genome_cluster_matrix.sum(axis=0).mean(),  # sum across clusters for each genome
        'max_clusters_per_genome': genome_cluster_matrix.sum(axis=0).max(),
        'min_clusters_per_genome': genome_cluster_matrix.sum(axis=0).min(),
        'total_genome_cluster_pairs': genome_cluster_matrix.sum().sum(),
        'clusters_in_all_genomes': (genome_cluster_matrix.sum(axis=1) == len(unique_genomes)).sum(),
        'clusters_in_single_genome': (genome_cluster_matrix.sum(axis=1) == 1).sum()
    }
    
    # Host matrix stats (note: axis changed since we transposed the matrix)
    host_stats = {
        'total_hosts': len(unique_hosts),
        'total_clusters': len(valid_clusters_sorted),
        'avg_hosts_per_cluster': host_cluster_matrix.sum(axis=1).mean(),    # sum across hosts for each cluster
        'max_hosts_per_cluster': host_cluster_matrix.sum(axis=1).max(),
        'min_hosts_per_cluster': host_cluster_matrix.sum(axis=1).min(),
        'avg_clusters_per_host': host_cluster_matrix.sum(axis=0).mean(),    # sum across clusters for each host
        'max_clusters_per_host': host_cluster_matrix.sum(axis=0).max(),
        'min_clusters_per_host': host_cluster_matrix.sum(axis=0).min(),
        'total_host_cluster_pairs': host_cluster_matrix.sum().sum(),
        'clusters_in_all_hosts': (host_cluster_matrix.sum(axis=1) == len(unique_hosts)).sum(),
        'clusters_in_single_host': (host_cluster_matrix.sum(axis=1) == 1).sum(),
        'host_specific_clusters': (host_cluster_matrix.sum(axis=1) == 1).sum()
    }
    
    # Save matrix statistics
    genome_stats_df = pd.DataFrame([genome_stats])
    genome_stats_file = os.path.join(output_dir, f'{method}_genome_matrix_stats.tsv')
    genome_stats_df.to_csv(genome_stats_file, sep='\t', index=False)
    
    host_stats_df = pd.DataFrame([host_stats])
    host_stats_file = os.path.join(output_dir, f'{method}_host_matrix_stats.tsv')
    host_stats_df.to_csv(host_stats_file, sep='\t', index=False)
    ...
    logging.info(f"Matrix stats - Avg genomes/cluster: {genome_stats['avg_genomes_per_cluster']:.1f}, "
                 f"Avg hosts/cluster: {host_stats['avg_hosts_per_cluster']:.1f}, "
                 f"Host-specific clusters: {host_stats['host_specific_clusters']}")
